...
bool convert<data_sdk::asset::AssetSpecificationQuery>::decode(
  ...
  query.required = std::pair{data_sdk::Symbol{
                                 node["ticker"].as<std::string>()},
                             epoch_core::AssetClassWrapper::FromString(
                                 node["class"].as<std::string>())};
  query.exchange =
      node["exchange"]
          ? epoch_core::ExchangeWrapper::FromString(node["exchange"].as<std::string>())
          : epoch_core::Exchange::Null;
  query.currency = node["currency"] ? epoch_core::CountryCurrencyWrapper::FromString(

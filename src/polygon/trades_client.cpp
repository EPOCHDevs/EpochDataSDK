...
public:
  ...
  getV3Trades(const std::string &ticker, std::optional<int> limit,
    ...
    return drogon::sync_wait(getV3TradesAsync(ticker, limit, timestamp_gte,
                                               timestamp_gt, timestamp_lte, timestamp_lt));
  }

  drogon::Task<Expected<epoch_frame::DataFrame>>
  getV3TradesAsync(std::string ticker, std::optional<int> limit,
                   std::optional<std::string> timestamp_gte,
                   std::optional<std::string> timestamp_gt,
                   std::optional<std::string> timestamp_lte,
                   std::optional<std::string> timestamp_lt) const {
...
drogon::Task<Expected<epoch_frame::DataFrame>>
TradesClient::getTradesAsync(std::string ticker, std::string from_date,
                             std::string to_date, std::optional<int> limit) const {
  return impl_->getV3TradesAsync(std::move(ticker), limit, std::move(from_date),
